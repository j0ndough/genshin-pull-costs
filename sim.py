...
def event_sim(base_rate: float, rate_mod: float, event_chance: float, soft_pity: int, hard_pity: int) -> dict:
    five_star_count = 0
    rate_up_count = 0
    ...
    offrate_check = False  # find a non event 5 star
    for x in range(samples):
        ...
        if random.random() >= 1 - current_rate or current_pity == hard_pity:  # roll a 5 star
            if random.random() >= event_chance or offrate_check:  # roll an event 5 star
                rate_up_count += 1
                event_pull_count.append(event_pity)
                event_pity = 0
                offrate_check = False
            else:   # roll a non event 5 star (next one is guarantee)
                offrate_check = True
            five_star_count += 1
...
def main():
    ...
    avgs = event_sim(base_rate, rate_mod, event_chance, soft_pity, hard_pity)
